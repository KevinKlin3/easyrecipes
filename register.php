...
$firstname_error = $lastname_error  =$nickname_error = $email_error =$invalid_image = NULL;
$password = NULL; $password_verify = NULL;
$password_error = NULL;
$password_match_error  = NULL;

$fileinfo = $image_name = $pageContent = $message =NULL;
			...
			if ($_FILES['profile_image']['error'] > 0) {
				...
				if (file_exists("$file")) {
					...
					if (move_uploaded_file($_FILES['profile_image']['tmp_name'], "$file" )){
						...
						///change
						$query = "INSERT INTO `user_table` VALUES (DEFAULT,'0','$firstname','$lastname','$username','$email','$password', '$image_name');";
...
$pageContent .=<<<HERE
	<main class="container p-5 my-5 bg-light text-secondary">
		$message
		<h1 class="ml-2"> Welcome, $firstname  $lastname </h1>
		<div class="row bg-light">
			<div class="col-sm">
				<img src ="images/$image_name" alt= "Profile image" class="profile_image" id="imageThumbnail">
			</div>
			<div class="col-sm">
				<h4><a href="profile.php?userID=$userID"> View Profile </a></h4>
				<p>Email:  $email </p>
...
$pageContent .=<<<HERE
	<main class="container bg-light">
		<fieldset>
		<h1 class="mt-2 text-center"> Easy Recipe Registration </h1>
			<form method="post" enctype="multipart/form-data" action="register.php">
				...
				<div class="mb-3 mt-3">
					...
					<input type="password" placeholder="re-type password" name="password_verify" id="password_verify" value="" class="form-control">$password_match_error
				</div>
				...
				<div class="mb-3 mt-3">
					<button class="btn btn-outline-primary" type="submit" name="submit" value="Submit ">Submit Application</button>
					<button class="btn btn-outline-danger" type="submit" name="reset" value="Reset ">Reset</button>
				</div>
			</form>
		</fieldset>
	</main>
