...
if ($recipeID) {
	$stmt = $conn->stmt_init();
   if ($stmt->prepare("SELECT `recipeTitle`, `recipeContent`, `authorID`, `recipeImage`, `date`, `type` FROM `recipe_table` WHERE `recipeID` = ?")) {
      $stmt->bind_param("i", $recipeID);
      $stmt->execute();
      $stmt->bind_result($recipeTitle, $recipeContent, $userID, $recipeImage, $date, $type);
      $stmt->fetch();
      $stmt->close();
   }
...
if (!empty($_FILES['recipeImage']['name'])) {
   ...
   if($valid)  {
      if(filter_has_var(INPUT_POST, 'insert'))  {
         $stmt = $conn->stmt_init();
         if ($stmt->prepare("INSERT INTO `recipe_table`(`recipeTitle`, `recipeContent`, `authorID`, `recipeImage`, `date`, `type` ) VALUES (?, ?, ?, ?, ?, ?)")) {
            $stmt->bind_param("ssi", $recipeTitle, $recipeContent, $userID, $recipeImage, $date, $type);
            $stmt->execute();
            $stmt->close();
         }
      ...
      if(filter_has_var(INPUT_POST, 'update'))  {
         ...
         if ($stmt->prepare("UPDATE `recipe_table` SET `recipeTitle`= ?, `recipeContent`= ?, `type`= ? WHERE `recipeID` = ?")) {
            $stmt->bind_param("ssi", $recipeTitle, $recipeContent, $recipeID, $type);
            $stmt->execute();
            $stmt->close();
         }
         header ("Location: ..\..\newRecipe.php?recipeID=$recipeID");
